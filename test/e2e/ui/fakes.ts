...
/** What the real panel last answered to a read of the server's add-ons, packs, pre-generation or friends' pack. */
  ...
  await page.route(`${origin}/api/**`, async (route: Route, request: Request) => {
    ...
    if (method === 'GET' || method === 'HEAD' || planning) {
      ...
      if (laid !== undefined) {
        ...
        if (/^\/api\/servers\/\w+\/(addons(\/checks)?|datapacks|resourcepack|pregen|mods\/share)$/.test(path)) state.reads.set(path, laid as Record<string, unknown>)
      ...
      if (res.ok()) {
        ...
        if (/^\/api\/servers\/\w+\/(addons(\/checks)?|datapacks|resourcepack|pregen|mods\/share)$/.test(path)) state.reads.set(path, await res.json().catch(() => ({})))
