...
export async function installFakes(page: Page, baseURL: string, view: () => View = () => 'live'): Promise<{ calls: ApiCall[]; unfaked: string[]; unrecorded: string[] }> {
  ...
  const state: FakeState = { origin, prefs: {}, backups: new Map(), update: {}, reads: new Map(), discord: { connected: false, alerts: [], liveStatus: true, delivery: {}, kinds: [] }, opSeq: 0, inviteSeq: 0, addresses: new Map(), maps: new Map(), imports: new Map(), ops: new Map(), schedules: new Map(), backupRules: new Map(), offsite: new Map(), sshKeys: new Set(), view, phases: new Map(), jobs: new Map() }
  ...
  await page.route(`${origin}/api/**`, async (route: Route, request: Request) => {
    ...
    if (method === 'GET' || method === 'HEAD' || planning) {
      ...
      if (fakeOp?.[1]) {
        ...
        const ended = state.jobs.get(fakeOp[1]) ?? (o ? finished(o) : undefined)
