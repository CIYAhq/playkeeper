...
prepare  on the release installed from GitHub: first-run setup, a server with
         ...
         console) and a backup. Records them in OUT/before.json.
...
def record(c):
    ...
    return {
        ...
        "backups": sorted([b["id"], b["sha256"], b["sizeBytes"]] for b in c.ok("GET", "/api/backups")),
...
def verify(a):
    ...
    check(st.get("agentVersion") == a.expect_version, f"the agent runs {st.get('agentVersion')}")
    ...
    for b in c.ok("GET", "/api/backups"):
        v = c.ok("POST", f"/api/backups/{b['id']}/verify", {})
...
def wait_result(c, version, timeout):
    ...
    while time.time() < deadline:
        try:
            status, info = c.request("GET", "/api/update", timeout=15)
...
def update(a):
    ...
    info = c.ok("POST", "/api/update/check", {})
    ...
    st = c.ok("GET", "/api/server")
    check(st.get("updateAvailable") == a.to, f"the dashboard's banner names {st.get('updateAvailable')}")
    step(f"Install Playkeeper {a.to} from the dashboard (expected: {a.expect})")
    t0 = time.time()
    op = c.ok("POST", "/api/update/apply", {"version": a.to})
    ...
    done = c.ok("GET", f"/api/operations/{op['id']}")
