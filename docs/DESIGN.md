...
- **Loading.** While data loads, grey shapes stand in for it in the same boxes (rows, table cells, cards, meters, chart bars, log lines), so nothing jumps when it arrives; never a bare "Loading…". The shapes pulse slowly and are hidden from screen readers, which hear "Loading…" once per group. Headings that don't depend on the data show straight away. Shapes are in `web/src/components/app/skeletons.tsx`.
