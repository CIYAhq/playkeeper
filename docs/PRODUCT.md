...
3. **Create:** accept the Minecraft EULA explicitly, choose a Paper/Minecraft version (checksum-verified; the newest stable one preselected) and RAM budget, start one server. Show downloading/starting/reachable/joinable and actionable failures. Copy a real join address. A real client joins.
