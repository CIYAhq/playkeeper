...
import type { Activity, Crash, LagStatus, LogsResponse, MachineView, RestorePreview, ServerStatus, SessionsResponse } from '@/api/types'
...
import { createStepOf, failedJob, isSettingUp, phaseLabel, statusTone, whyNot } from '@/lib/phase'
...
export function Overview({ server }: { server: ServerStatus }) {
  ...
  if (!stale && statusTone(server) === 'crashed' && !server.operation) return <CrashedView server={server} />
  ...
  return (
    <Card className="relative transition-[box-shadow,border-color] duration-(--motion-fast) ease-standard focus-within:border-primary/40 hover:border-primary/40 hover:shadow-lift">
      <div className="flex items-center justify-between gap-3">
        ...
        {place.route && (
          <a {...linkProps(place.route)} className="relative z-10 inline-flex items-center gap-1 text-xs font-medium text-primary hover:underline max-sm:text-[15px]">
            {place.name}
...
function CrashedView({ server: s }: { server: ServerStatus }) {
  ...
  const button = (
    <Button className={cn('w-full', phone && 'text-base')} size={phone ? 'touch' : 'lg'} loading={busy} onClick={act} disabledReason={whyNot(s, 'start', place.offline)}>
      <PlayIcon />
      {choice?.button}
    </Button>
  )
