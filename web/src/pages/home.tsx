...
export function HomePage() {
      ...
      <>
        <PageHeader
          ...
          subtitle={
            phone
              ? undefined
              : t("home.emptySubtitle", { machine: ws.machineName })
          }
          phoneAction={<PhoneMoreButton />}
        />
        <PageBody className="flex flex-1 flex-col items-center pt-10 text-center max-sm:pt-0">
          <Pip pose="wave" size={phone ? 104 : 96} />
          <h2 className="mt-4 text-title font-extrabold tracking-[-0.015em]">
            {t("home.emptyTitle")}
          </h2>
          <p className="mt-2 max-w-[420px] text-sm text-muted-foreground max-sm:text-[15px]">
            {t("home.emptyBody")}
          </p>
          {phone ? (
            <Button
              ...
              render={<a {...linkProps({ name: "new-server" })} />}
            >
              <PlusIcon />
              {t("checklist.create")}
            </Button>
  ...
  if (servers && grouped)
    subtitle = `${t("machines.home.subtitle", { count: servers.length, machines: ws.machines.length })}${t("common.dot")}${t("machines.home.playing", { count: players })}`;
  else if (servers)
    subtitle = `${t("home.servers", { count: servers.length, machine: ws.machineName })}${t("common.dot")}${t("home.playing", { count: players })}`;
  return (
    <>
      <PageHeader
        title={t("home.title")}
      ...
      <PageBody className="flex flex-col gap-4">
        ...
        {grouped ? (
          byMachine(servers ?? [], ws.machines).map((g) => (
            <section
              key={g.machine.id}
              aria-labelledby={`on-${g.machine.id}`}
              ...
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {g.servers.map((s) => (
                  <ServerCard
                    key={s.id}
                    server={s}
                    update={newerStable(s.config, catalog?.versions)}
                  />
                ))}
        ...
        <div className="grid gap-4 lg:grid-cols-[1.6fr_1fr]">
          <Card>
            ...
            <ActivityList
              items={activity.data}
              servers={servers ?? []}
              empty={t("home.activityEmpty")}
              className="mt-3 flex-1"
            />
            <a
              {...linkPath("/settings#audit")}
...
async function recentActivity(machines: string[]): Promise<Activity[]> {
  const got = await Promise.allSettled(
    machines.map((id) => get<Activity[]>(machineApi(id, "/activity?limit=5"))),
  );
...
function MachineNotice() {
  ...
  if (ws.agentDown)
    return (
      <Notice tone="error" title={t("agentDown.title")}>
        {t("agentDown.note")}
      </Notice>
    );
  if (disk)
    return (
      <Notice
        tone={disk.status === "fail" ? "error" : "warning"}
        title={t("overview.lowDiskTitle", { detail: disk.detail })}
      >
        {disk.fix}
      </Notice>
    );
...
function CardDetail({ server: s }: { server: ServerStatus }) {
  const ws = useWorkspace();
  if (isStale(s, ws.stale) || reachOf(s, ws).state !== "live")
    return (
    ...
    case "crashed":
      return (
        <>
          ...
          {!s.operation && (
            <StartButton server={s} label={t("server.startAgain")} />
          )}
        </>
      );
    case "busy":
        ...
        <>
          ...
          <span className="text-[13px] text-muted-foreground">
            {s.stoppedAt
              ? t("card.napping", {
                  duration: formatSpan(
                    (Date.now() - new Date(s.stoppedAt).getTime()) / 1000,
                  ),
                })
              : t("card.stoppedNever")}
          </span>
          {s.exists && !s.operation && s.phase === "stopped" && (
            <StartButton server={s} label={t("card.start")} />
          )}
        </>
      );
    default: {
...
function ServerCard({
  ...
  const address = joinAddress(
    joinHost(machineOf(s, ws.machines), window.location.hostname),
    s.gamePort,
  );
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h2 className="truncate text-[17px] leading-[22px] font-bold">
            <a
              {...linkProps({ name: "server", slug: s.slug, tab: "overview" })}
...
function NewServerCard({ machine }: { machine?: MachineView }) {
  ...
  const cls =
    "flex min-h-[176px] flex-col items-center justify-center rounded-3xl border border-dashed border-input bg-warm p-4 text-center outline-none";
  ...
  return (
    <a
      {...linkProps(
        machine
          ? { name: "new-server", machine: machine.id }
          : { name: "new-server" },
      )}
      className={cn(
        cls,
    ...
    >
      ...
      <span className="mt-3 text-[15px] font-semibold">
        {machine ? t("machines.newServerHere") : t("nav.newServer")}
      </span>
      {live && (
        <span className="mt-1 text-xs text-muted-foreground">
          {full
            ? t("home.newServerFull", { machine: name })
            : t("home.newServerFree", {
                memory: formatMB(live.memoryFreeMB),
                machine: name,
              })}
        </span>
      )}
...
function MachineHeading({ machine: m }: { machine: MachineView }) {
  ...
  if (m.kind === "local")
    word =
      machineState(m, ws).tone === "good"
        ? t("machines.home.healthy")
        : ws.agentDown
          ? t("machines.home.notAnswering")
          : t("status.docker");
    ...
    if (m.error || !m.live) word = t("machines.home.notAnswering");
      ...
      word = sameDay(new Date(link.connectedAt), new Date(now))
        ? t("machines.home.connectedToday")
        : t("machines.home.connectedOn", {
            date: formatDate(link.connectedAt),
          });
  } else if (link?.state === "waiting") word = t("machines.home.waiting");
  else
    word = link?.lastSeen
      ? t("machines.home.away", { duration: awayLong(link.lastSeen, now) })
      : t("machines.offline");
  const meta = m.live
    ? [m.live.os, formatMB(m.live.memoryTotalMB), word]
    : [word];
  return (
    <h2
      id={`on-${m.id}`}
      className="flex flex-wrap items-baseline gap-x-2 text-[15px] font-semibold"
    >
      <ServerIcon
      ...
      <a
        {...linkProps(
          m.kind === "local"
            ? { name: "machine", id: m.id }
            : { name: "machine-settings", id: m.id },
        )}
...
function MachineCard() {
  ...
  const diskUsed =
    live?.diskTotalBytes && live.diskFreeBytes !== undefined
      ? ((live.diskTotalBytes - live.diskFreeBytes) / live.diskTotalBytes) * 100
      : undefined;
  return (
    <Card>
      ...
      {live && (
        <CardHint>
          {t("home.machineMeta", {
            os: live.os,
            memory: formatMB(live.memoryTotalMB),
          })}
        </CardHint>
      )}
      {live ? (
        <div className="mt-4 flex flex-col gap-4">
          <MeterRow
            label={t("home.memoryReserved")}
            value={t("home.ofTotal", {
              used: formatMB(reserved),
              total: formatMB(live.memoryTotalMB),
            })}
            percent={
              live.memoryTotalMB ? (reserved / live.memoryTotalMB) * 100 : 0
            }
          />
          <MeterRow
            label={t("home.cpu")}
            value={formatPercent(live.cpuPercent)}
            percent={live.cpuPercent}
          />
          <MeterRow
            label={t("home.disk")}
            value={t("home.diskFree", {
              free: formatBytes(live.diskFreeBytes),
            })}
            percent={diskUsed}
          />
        </div>
        ...
        <p className="mt-3 flex-1 text-[13px] text-muted-foreground">
          {ws.agentDown ? t("nav.notAnswering") : t("common.loading")}
        </p>
      )}
      {m && (
        <a
          {...linkProps({ name: "machine", id: m.id })}
          className="mt-auto inline-flex items-center gap-1 self-start pt-4 text-xs font-medium text-primary hover:underline"
        >
