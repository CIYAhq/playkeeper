...
function PhoneShell({ route, overlays, children }: { route: Route; overlays: ReactNode; children: ReactNode }) {
  ...
  const current: ServerTab | 'more' | undefined = route.name === 'server' ? (route.tab === 'settings' || route.tab === 'map' || route.tab === 'plugins' || route.tab === 'mods' ? 'more' : route.tab) : route.name === 'more' ? 'more' : undefined
