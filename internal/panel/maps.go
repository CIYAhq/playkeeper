...
// accounts, and serves the shared map at /map/<slug> to anyone while its
// switch is on.
...
// shared, a stopped server, a wrong link and an agent that doesn't answer
// all get the same 404, which never names a server.
...
// sharedMap asks the agent for part of the shared map under slug: "" for
// the page's details, or worlds, players, a tile or the icon. It reports
// false for anything but a complete answer of the expected type.
func (s *Server) sharedMap(r *http.Request, slug, part string) (contentType string, body []byte, ok bool) {
	if !reMapSlug.MatchString(slug) {
		return "", nil, false
	}
	p := "/v1/public-maps/" + slug
	if part != "" {
		p += "/" + part
	}
...
// hMapPage serves the UI for /map/<slug>: 200 while the map is shared,
// otherwise 404 with the same page, which then says the map isn't
// available.
func (s *Server) hMapPage(w http.ResponseWriter, r *http.Request, _ *session) {
	...
	if _, _, ok := s.sharedMap(r, r.PathValue("slug"), ""); ok {
...
func (s *Server) publicMap(part func(r *http.Request) (string, bool)) func(http.ResponseWriter, *http.Request, *session) {
	return func(w http.ResponseWriter, r *http.Request, _ *session) {
		...
		ct, body, ok := s.sharedMap(r, r.PathValue("slug"), p)
		if !ok {
			writeMapUnavailable(w)
			return
		}
...
// now, which it only does while its players switch is on. Faces come from
// the panel's cache, so viewers' addresses never reach Mojang.
func (s *Server) hPublicMapFace(w http.ResponseWriter, r *http.Request, _ *session) {
	if !s.publicMapAllowed(w, r) {
		return
	}
	slug, name := r.PathValue("slug"), r.PathValue("name")
	if !reMapSlug.MatchString(slug) || !minecraft.ValidPlayerName(name) {
		writeMapUnavailable(w)
		return
	}
	var listed struct {
	...
	if status, err := s.agent.Do(r.Context(), "GET", "/v1/public-maps/"+slug+"/players", nil, nil, &listed); err != nil || status != http.StatusOK {
		writeMapUnavailable(w)
		return
	}
