...
func backup(t *testing.T, id, when string, edit ...func(*Backup)) Backup {
	b := Backup{ID: id, Kind: KindScheduled, CreatedAt: at(t, when), SizeBytes: gib, MinecraftVersion: "1.21.8", LevelName: "world", Verified: passedCheck(), OnHost: true}
	for _, e := range edit {
		e(&b)
	}
	return b
...
func TestDefaultRulesKeepAboutADozenDailyBackups(t *testing.T) {
	...
	want := []string{
		...
		"d2026-09-13", "d2026-09-06", "d2026-08-31", "d2026-07-31",
	}
	if got := keptIDs(res.OnHost); !slices.Equal(got, want) {
		t.Fatalf("kept %v, want %v", got, want)
	}
	if got, want := res.OnHost.Summary.Text, "Keeps 11 backups on this machine (11.0 GiB): the newest 3, one a day for a week, one a week for 4 weeks and one a month for 3 months. Deletes 109 (109.0 GiB)."; got != want {
		t.Fatalf("summary\n got %q\nwant %q", got, want)
	}
	if p := res.OnHost.Summary.Params; p["keep"] != "11" || p["delete"] != "109" || p["deleteBytes"] != fmt.Sprint(109*gib) {
	...
	if got := codes(newest); !slices.Equal(got, []string{"last", "daily", "weekly", "monthly", "newest"}) {
	...
	if got := reason(t, decision(t, res.OnHost, "d2026-07-31"), "monthly"); got.Text != "The newest backup of July 2026 (one a month for 3 months)." || got.Params["month"] != "2026-07" {
		t.Fatalf("monthly reason %+v", got)
	}
	...
	for id, want := range map[string]Text{
		...
		"d2026-08-30": {Code: "same_month", Text: "Another backup from August 2026 is kept (one a month for 3 months)."},
		"d2026-06-30": {Code: "beyond_rules", Text: "Older than what the rules keep on this machine: the newest 3, one a day for a week, one a week for 4 weeks and one a month for 3 months."},
	} {
...
func TestManyBackupsADayKeepTheNewestOfEachDay(t *testing.T) {
	...
	if r.Params["kept"] != "h09-24T23" || r.Params["date"] != "2026-09-24" || r.Text != "Another backup from Thursday 24 September 2026 is kept (one a day for a week)." {
...
func TestPinnedBackupsAreNeverDeleted(t *testing.T) {
	...
	if got, want := res.OnHost.Summary.Text, "Keeps 3 backups on this machine (3.0 GiB): the newest 2, plus 1 pinned. Deletes 27 (27.0 GiB)."; got != want {
		t.Fatalf("summary\n got %q\nwant %q", got, want)
	}
}
...
func TestFailedChecks(t *testing.T) {
	t.Run("the newest checked backup stays when newer ones failed", func(t *testing.T) {
		...
		if got, want := res.OnHost.Summary.Text, "Keeps 3 backups on this machine (3.0 GiB): the newest, plus 2 whose checks failed. Deletes 2 (2.0 GiB)."; got != want {
			t.Fatalf("summary\n got %q\nwant %q", got, want)
		}
	})
...
func TestRollbackArchivesARestoreOrUpdateStillNeedsAreKept(t *testing.T) {
	...
	if got, want := res.OnHost.Summary.Text, "Keeps 2 backups on this machine (2.0 GiB): the newest, plus 1 a restore or update still needs. Deletes 8 (8.0 GiB)."; got != want {
		t.Fatalf("summary\n got %q\nwant %q", got, want)
	}
...
func TestTheWorldBeforeARestoreIsKeptUntilItHasAnotherCopy(t *testing.T) {
	for _, kind := range []string{KindBeforeRestore, KindRollback} {
		t.Run(kind, func(t *testing.T) {
			...
			if got, want := res.OnHost.Summary.Text, "Keeps 3 backups on this machine (3.0 GiB): the newest 2, plus 1 that is the only copy of an earlier world. Deletes 7 (7.0 GiB)."; got != want {
				t.Fatalf("summary\n got %q\nwant %q", got, want)
			}
...
func TestBothPlacesTogetherNeverDeleteTheOnlyCopy(t *testing.T) {
	...
	if got, want := res.OffSite.Summary.Text, "Keeps 4 backups off the server (4.0 GiB): the newest 3, plus 1 that is the only copy of an earlier world. Deletes 6 (6.0 GiB)."; got != want {
...
func TestBackupsMadeByHand(t *testing.T) {
	...
	if got, want := res.OnHost.Summary.Text, "Keeps 4 backups on this machine (4.0 GiB): the newest and one a day for 2 days, plus 2 made by hand. Deletes 1 (1.0 GiB)."; got != want {
		t.Fatalf("summary\n got %q\nwant %q", got, want)
	}
...
func TestKeepAll(t *testing.T) {
	...
	res := Decide(bs, s, nil)
	if res.OnHost.Delete != 0 || res.OnHost.Summary.Text != "Keeps all 5 backups on this machine (5.0 GiB)." {
...
func TestOutOfRangeRulesAreClampedSoTheNewestAlwaysStays(t *testing.T) {
	bs := daily(t, "2026-09-25", 3, func(b *Backup) { b.Verified = nil })
	s := DefaultSettings()
	s.OnHost = Rules{Last: 0, Daily: -4}
	if got := keptIDs(Decide(bs, s, nil).OnHost); !slices.Equal(got, []string{"d2026-09-25"}) {
		t.Fatalf("kept %v", got)
	}
}
...
func TestValidate(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{func(s *Settings) { s.OnHost.Last = 0 }, "onHost.last", "The number of newest backups to keep on this machine must be between 1 and 1000, not 0."},
...
func TestDescribe(t *testing.T) {
	...
	want := []string{
		"Keeps the newest 3, one a day for a week, one a week for 4 weeks and one a month for 3 months on this machine, at most 17 backups besides the ones that are always kept.",
		"Keeps the newest 3, one a day for 2 weeks, one a week for 8 weeks and one a month for a year off the server, at most 37 backups besides the ones that are always kept.",
		"Days, weeks and months without a backup don't count, so a pause in backups never makes the rules delete older ones.",
		"Always kept: pinned backups, the newest backup that passed its check, backups a restore or update may still need, backups made by hand and backups that are the only copy of an earlier world (from before a restore, or the last one on an earlier Minecraft version or of another world) until they are downloaded or copied off the server.",
	}
	if len(got) != len(want) {
	...
	if got[0].Params["daily"] != "7" || got[1].Params["where"] != "off-site" {
		t.Errorf("params %v %v", got[0].Params, got[1].Params)
	}

	s := Settings{OnHost: Rules{KeepAll: true, Last: 1}, OffSite: Rules{Last: 1, Weekly: 52, Monthly: 24}, IncludeManual: true, DeleteOnlyCopies: true}
	got = s.Describe()
	for i, want := range []string{
		"Keeps every backup on this machine.",
		"Keeps the newest, one a week for a year and one a month for 2 years off the server, at most 77 backups besides the ones that are always kept.",
		"",
		"Always kept: pinned backups, the newest backup that passed its check and backups a restore or update may still need.",
	} {
		if want != "" && got[i].Text != want {
			t.Errorf("text %d\n got %q\nwant %q", i, got[i].Text, want)
		}
	}
	if s.OnHost.MaxKept() != -1 || DefaultSettings().OnHost.MaxKept() != 17 {
		t.Errorf("MaxKept")
	}
	if !strings.HasPrefix(dailyPhrase(1), "one a day for 1 day") || dailyPhrase(14) != "one a day for 2 weeks" || dailyPhrase(30) != "one a day for 30 days" {
		t.Errorf("daily phrases %q %q %q", dailyPhrase(1), dailyPhrase(14), dailyPhrase(30))
	}
}
