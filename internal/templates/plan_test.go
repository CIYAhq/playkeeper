...
func TestPlanTypes(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "Forge at its version", template: ofType("forge", nil, false), catalog: withType(paperCatalog(), "forge", "26.2"),
			typ: TypeChoice{ID: "forge", Name: "Forge"}, version: "forge-26.2"},
		{name: "Forge that this Playkeeper cannot create yet", template: ofType("forge", nil, false), catalog: withType(paperCatalog(), "neoforge", "26.2"),
			typ: TypeChoice{ID: "forge", Name: "Forge"}, blockers: []Kind{KindTypeUnavailable},
			msg: "This Playkeeper cannot create Forge servers yet."},
