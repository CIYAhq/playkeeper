...
func TestVoiceChatOpensItsPortAndClosesItWhenRemoved(t *testing.T) {
	...
	if got := published(); !slices.Equal(got, []string{"24455/udp→24455", "25565/tcp→" + strconv.Itoa(e.srv().gamePort)}) {
	...
	if got := published(); !slices.Equal(got, []string{"25565/tcp→" + strconv.Itoa(e.srv().gamePort)}) {
