...
- **Free name:** type a name (3 to 32 lowercase letters, numbers or single dashes) and **Claim** it. Playkeeper's names service points it at the VPS's public IP and adds the DNS record Let's Encrypt checks, so no extra port is needed; it takes about a minute. Each server then joins at `<server>.yourname.playkeeper.io` with no port, and the dashboard is at `https://yourname.playkeeper.io:8443`, where you sign in once more (sign-in belongs to one address). One name per install. **Release it** stops the addresses right away and holds the name for 30 days so nobody else takes over your players. After about a month with the VPS offline, a name stops updating until you press **Refresh**. If the names service can't be reached, Machine settings says so; addresses that already work keep working.
