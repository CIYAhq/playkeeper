...
- **Sleep when nobody's playing:** off until you turn it on in a server's Settings. After 5 minutes to 4 hours with nobody on (15 minutes by default), the server stops and frees its memory, and players see "Asleep · join to wake it". A join from someone on the allowlist or an operator wakes it in about 30 seconds, or wake it from the dashboard.
