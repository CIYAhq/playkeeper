...
**Plugins and mods:** The **Plugins** tab (**Mods** on Fabric, Quilt, NeoForge and Forge servers) searches [Modrinth](https://modrinth.com) and [Hangar](https://hangar.papermc.io) in one list that shows only what works on the server's type and Minecraft version. Installing shows first what will be added, including the dependencies it needs, and every file is checked against the checksum its library publishes before it goes into the server's folder. Updates go one at a time or all at once, each after the same preview, and a plugin can be removed with or without its settings. Plugins added by hand are listed too, and Playkeeper can take over the ones the library recognises so they get updates. New and updated plugins load when the server restarts.
...
**Start from your own world:** **New server** → **A world** (under **Start from**), or **Start from your own world** on a server's World tab, makes a server from a world you already have: your singleplayer world, or one from Aternos, Minehut, Realms or another host, with steps for getting it out. Upload it as one or more `.zip`, `.tar.gz` or `.tar` files, such as the separate Nether and End downloads some hosts give; the upload carries on where it stopped if the connection drops. Before anything changes, Playkeeper shows what's inside (the world, its dimensions and data packs, and the Minecraft version it was made in) and offers to upgrade it to the suggested version or keep its own, when Playkeeper can run that. The new server gets the world's seed, game mode, difficulty and hardcore setting, and its data packs stay on; plugins, mods and the old host's other settings stay behind. When the new version upgrades the world, the world as you uploaded it is kept as a backup. Leaving the page before the server is made cancels the upload, and so does a restart of Playkeeper.

**Map:** a server's **Map** tab (on every server type but Vanilla and Forge) shows its world from above, drawn by [squaremap](https://modrinth.com/plugin/squaremap) on your own server, with who's playing and where. **Turn on the map** installs squaremap, which the server loads with one restart, now or once nobody's playing; it then draws the land explored so far and keeps up as people explore. **Turn off the map**, in the map's menu, removes it and keeps the drawn map for next time or deletes it. The Plugins or Mods tab lists squaremap as used by the Map, and if it's deleted by hand, **Turn on the map** installs it again.

**Share the map:** **Share with a link** on the Map tab gives a link anyone can open without signing in. Switching sharing on makes a new link each time, so older ones stop working, and the link says the map isn't available while sharing or the map is off or the server is stopped. Player positions stay hidden unless you turn on **Show players on the shared map**; the drawn land, builds included, is visible to anyone with the link. The link uses the machine's name once it has one (Machine settings › Address); until then it uses the address the dashboard is open at, and your friends' browsers warn about the dashboard's self-signed certificate, as yours did.
